...
function woocommerce_bnpmercanetcw_meta_boxes(){
	global $post;
	...
	$transactions = array();
	try{
		$transactions = BNPMercanetCw_Util::getTransactionsByPostId($post->ID);
	}
	catch(Exception $e){
		//Ignore
	}
	if (count($transactions) > 0) {
		add_meta_box('woocommerce-bnpmercanetcw-information', 
				__('BNP Paribas Mercanet Transactions', 'woocommerce_bnpmercanetcw'), 
				'woocommerce_bnpmercanetcw_transactions', 'shop_order', 'normal', 'default');
	}
}
add_action('add_meta_boxes', 'woocommerce_bnpmercanetcw_meta_boxes');

function woocommerce_bnpmercanetcw_transactions($post){
	$transactions = BNPMercanetCw_Util::getTransactionsByPostId($post->ID);

	echo '<table class="wp-list-table widefat table bnpmercanetcw-transaction-table">';
	echo '<thead><tr>';
	echo '<th>#</th>';
	echo '<th>' . __('Transaction Number', 'woocommerce_bnpmercanetcw') . '</th>';
	echo '<th>' . __('Date', 'woocommerce_bnpmercanetcw') . '</th>';
	echo '<th>' . __('Payment Method', 'woocommerce_bnpmercanetcw') . '</th>';
	echo '<th>' . __('Is Authorized', 'woocommerce_bnpmercanetcw') . '</th>';
	echo '<th>' . __('Amount', 'woocommerce_bnpmercanetcw') . '</th>';
	echo '<th>&nbsp;</th>';
	echo '</tr></thead>';
	
	foreach ($transactions as $transaction) {
		echo '<tr class="bnpmercanetcw-main-row"  id="bnpmercanetcw-main_row_' . $transaction->getTransactionId() . '">';
		...
		echo '</td>';
		echo '<td>
				<a class="bnpmercanetcw-more-details-button button">' . __('More Details', 'woocommerce_bnpmercanetcw') . '</a>
				<a class="bnpmercanetcw-less-details-button button">' . __('Less Details', 'woocommerce_bnpmercanetcw') . '</a>
			</td>';
		echo '</tr>';
		echo '<tr class="bnpmercanetcw-details-row" id="bnpmercanetcw_details_row_' . $transaction->getTransactionId() . '">';
		echo '<td colspan="7">';
		echo '<div class="bnpmercanetcw-box-labels">';
		if ($transaction->getTransactionObject() !== null) {
			foreach ($transaction->getTransactionObject()->getTransactionLabels() as $label) {
				...
				if (isset($label['description']) && !empty($label['description'])) {
					echo woocommerce_bnpmercanetcw_get_help_box($label['description']);
				}
				echo '</div>';
			...
			echo __("No more details available.", 'woocommerce_bnpmercanetcw');
		}
		echo '</div>';
		...
		if ($transaction->getTransactionObject() !== null && $transaction->getTransactionObject()->isAuthorized()) {
			...
			if(!empty($instructions)){
				echo '<div class="bnpmercanetcw-payment-information">';
				echo '<b>'.__('Payment Information', 'woocommerce_bnpmercanetcw').'</b><br />';
				echo $instructions;
				echo '</div>';
			}
		}
		...
		if ($transaction->getTransactionObject() !== null) {
			...
			if ($transaction->getTransactionObject()->isCapturePossible()) {
				...
				$url = Customweb_Util_Url::appendParameters(get_admin_url() . 'admin.php', 
						array(
							'page' => 'woocommerce-bnpmercanetcw_capture',
			...
			if ($transaction->getTransactionObject()->isCancelPossible()) {
				...
				$url = Customweb_Util_Url::appendParameters(get_admin_url() . 'admin.php', 
						array(
							'page' => 'woocommerce-bnpmercanetcw_cancel',
							'cwTransactionId' => $transaction->getTransactionId(),
							'noheader' => 'true' 
						));
			...
			if ($transaction->getTransactionObject()->isRefundPossible()) {
				...
				$url = Customweb_Util_Url::appendParameters(get_admin_url() . 'admin.php', 
						array(
							'page' => 'woocommerce-bnpmercanetcw_refund',
							'cwTransactionId' => $transaction->getTransactionId(),
							'noheader' => 'true' 
						));
			...
			if (count($transaction->getTransactionObject()->getCaptures())) {
				...
				echo '<h4>' . __('Captures', 'woocommerce_bnpmercanetcw') . '</h4>';
				echo '<table class="table" cellpadding="0" cellspacing="0" width="100%">';
				echo '<thead>';
				echo '<tr>';
				echo '<th>' . __('Date', 'woocommerce_bnpmercanetcw') . '</th>';
				echo '<th>' . __('Amount', 'woocommerce_bnpmercanetcw') . '</th>';
				echo '<th>' . __('Status', 'woocommerce_bnpmercanetcw') . '</th>';
				echo '</tr>';
				echo '</thead>';
				echo '<tbody>';
			...
			if (count($transaction->getTransactionObject()->getRefunds())) {
				...
				echo '<h4>' . __('Refunds', 'woocommerce_bnpmercanetcw') . '</h4>';
				echo '<table class="table" cellpadding="0" cellspacing="0" width="100%">';
				echo '<thead>';
				echo '<tr>';
				echo '<th>' . __('Date', 'woocommerce_bnpmercanetcw') . '</th>';
			...
			if (count($transaction->getTransactionObject()->getHistoryItems())) {
				...
				echo '<h4>' . __('Previous Actions', 'woocommerce_bnpmercanetcw') . '</h4>';
				echo '<table class="table" cellpadding="0" cellspacing="0" width="100%">';
				echo '<thead>';
				echo '<tr>';
				echo '<th>' . __('Date', 'woocommerce_bnpmercanetcw') . '</th>';
				echo '<th>' . __('Action', 'woocommerce_bnpmercanetcw') . '</th>';
				echo '<th>' . __('Message', 'woocommerce_bnpmercanetcw') . '</th>';
				echo '</tr>';
				echo '</thead>';
				echo '<tbody>';
...
function woocommerce_bnpmercanetcw_get_help_box($text){
	return '<img class="help_tip" data-tip="' . $text . '" src="' . BNPMercanetCw_Util::getResourcesUrl('image/help.png') . '" height="16" width="16" />';
}
