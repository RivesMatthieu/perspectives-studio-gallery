...
class BNPMercanetCw_RecurringTransactionContext extends BNPMercanetCw_TransactionContext implements Customweb_Payment_Authorization_Recurring_ITransactionContext
{
	private $initialTransactionId;
	
	private $initialTransaction;
	
	public function __construct(BNPMercanetCw_Entity_Transaction $transaction, BNPMercanetCw_RecurringOrderContext $orderContext) {
		parent::__construct($transaction, $orderContext);
		$initialTransactionId = $orderContext->getInitialTransactionId();
		$initialTransaction = BNPMercanetCw_Util::getTransactionById($initialTransactionId);
	...
	public function getInitialTransaction() {
		if ($this->initialTransaction === null) {
			$this->initialTransaction = BNPMercanetCw_Util::getTransactionById($this->initialTransactionId);
		}
