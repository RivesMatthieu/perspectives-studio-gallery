...
class BNPMercanetCw_Entity_ExternalCheckoutContext extends Customweb_Payment_ExternalCheckout_AbstractContext {
	...
	protected function loadPaymentMethodByMachineName($machineName){
		$paymentMethods = BNPMercanetCw_Util::getPaymentMethods(true);
		foreach($paymentMethods as $method) {
			$instance = BNPMercanetCw_Util::getPaymentMehtodInstance($method);
	 ...
	 * @return BNPMercanetCw_Entity_ExternalCheckoutContext
	 */
	public static function getContextById($id, $loadFromCache = true) {
		return BNPMercanetCw_Util::getEntityManager()->fetch('BNPMercanetCw_Entity_ExternalCheckoutContext', $id, $loadFromCache);
	}
	...
	public static function getReusableContextFromCookie($loadFromCache = true) {
		if (isset($_COOKIE['bnpmercanetcw-woocommerce-context-id']) && isset($_COOKIE['bnpmercanetcw-woocommerce-context-key'])) {
			try {
				$result = BNPMercanetCw_Util::getEntityManager()->searchByFilterName('BNPMercanetCw_Entity_ExternalCheckoutContext', 'loadContextCookie', array('>contextId' => $_COOKIE['bnpmercanetcw-woocommerce-context-id'], '>cookieKey' => $_COOKIE['bnpmercanetcw-woocommerce-context-key']), $loadFromCache);
				if (count($result) > 0) {
					$context = current($result);
					if ($context instanceof BNPMercanetCw_Entity_ExternalCheckoutContext && $context->getState() == Customweb_Payment_ExternalCheckout_IContext::STATE_PENDING) {
						return $context;
					}
				}
			}
	...
	public function updateFromCart($cart) {
		...
		if(BNPMercanetCw_ConfigurationAdapter::getExternalCheckoutPlacement() == 'checkout') {
		...
		$this->setInvoiceItems(BNPMercanetCw_CartUtil::getInoviceItemsFromCart($cart));
