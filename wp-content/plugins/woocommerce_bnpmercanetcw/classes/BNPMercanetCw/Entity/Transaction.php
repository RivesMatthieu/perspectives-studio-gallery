 ...
 *
 *
 * @Entity(tableName = 'woocommerce_bnpmercanetcw_transactions')
 * @Filter(name = 'loadByPostId', where = 'postId = >postId', orderBy = 'postId')
 */
class BNPMercanetCw_Entity_Transaction extends Customweb_Payment_Entity_AbstractTransaction {
	...
	protected function updateOrderStatus(Customweb_Database_Entity_IManager $entityManager, $orderStatus, $orderStatusSettingKey){
		...
		$this->changeLanguage();		
		$GLOBALS['woocommerce_bnpmercanetcw__status_change'] = true;
		$order = $this->getOrder();
		$paymentMethod = BNPMercanetCw_Util::getPaymentMehtodInstance($this->getPaymentClass());
		if ($orderStatusSettingKey != 'status_authorized' || $paymentMethod->getPaymentMethodConfigurationValue('status_authorized') != 'use-default') {
			$order->update_status($orderStatus, __('Payment Notification', 'woocommerce_bnpmercanetcw'));
		}
		//Restore the original language
		$this->restoreLanguage();
		unset($GLOBALS['woocommerce_bnpmercanetcw__status_change']);
	}
	...
	protected function authorize(Customweb_Database_Entity_IManager $entityManager){
		if ($this->getTransactionObject()->isAuthorized()) {
			...
			//Switch language to the transaction language so emails are translated correctly
			$this->changeLanguage();
			delete_post_meta( $orderPostId, '_bnpmercanetcw_pending_state' );
			...
			if ($orderStatus != 'use-default') {
				$GLOBALS['woocommerce_bnpmercanetcw_complete_order_status'] = $orderStatus;
				add_filter('woocommerce_payment_complete_order_status', 'woocommerce_bnpmercanetcw_complete_order_status', 10 , 3);
			}
	...
	protected function forceTransactionFailing(){
		...
		if(BNPMercanetCw_Util::getAuthorizedTransactionByPostId($this->getPostId()) !== null){
			//Another Transaction has already sucessfuly authorized this transaction, do not mark the order as cancelled
			return;
		}
		$this->getOrder()->add_order_note(__('Error Message: ', 'woocommerce_bnpmercanetcw') . $message);
		$orderId= $this->getOrder()->get_id();
		delete_post_meta( $orderId, '_bnpmercanetcw_pending_state' );
		...
		if ($this->getAuthorizationType() != Customweb_Payment_Authorization_Recurring_IAdapter::AUTHORIZATION_METHOD_NAME && $this->getTransactionObject()->getTransactionContext()->createRecurringAlias()) {
			...
			add_action( 'woocommerce_email', 'woocommerce_bnpmercanetcw_unhook_subscription_cancel' );
		}
	...
	private function changeLanguage(){
		...
		$GLOBALS['woo_bnpmercanetcwAuthorizeLanguage'] = $this->getTransactionObject()->getTransactionContext()->getOrderContext()->getLanguage()->getIetfCode();
		add_filter('locale', 'woocommerce_bnpmercanetcw_locale');
	}
	
	private function restoreLanguage(){
		
		do_action( 'wpml_switch_language', null);
		remove_filter('locale', 'woocommerce_bnpmercanetcw_locale');
	}
