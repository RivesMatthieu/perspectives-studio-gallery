 ...
 */
class BNPMercanetCw_LoggingListener implements Customweb_Core_Logger_IListener {
	...
	public function addLogEntry($loggerName, $level, $message, Exception $e = null, $object = null){
		...
		self::$logger->add('BNPMercanetCw', $content);
	}

	private function isLevelActive($level){
		switch (BNPMercanetCw_ConfigurationAdapter::getLoggingLevel()) {
			case 'debug':
				return true;
			case 'info':
