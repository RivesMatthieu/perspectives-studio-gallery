...
class BNPMercanetCw_Migration_1_1_0 implements Customweb_Database_Migration_IScript {

	public function execute(Customweb_Database_IDriver $driver){
		global $wpdb;	

		$query = "SHOW TABLES IN `".DB_NAME."` LIKE  'bnpmercanetcw_storage';";
		...
		if(is_array($select) && count($select) > 0) {
			...
			$tableName = $entityManager->getTableNameForEntityByClassName('BNPMercanetCw_Entity_Storage');
			$driver->query("RENAME TABLE `bnpmercanetcw_storage` TO `" . $tableName . "`")->execute();
		}
		else {
			$driver->query(
			
				"CREATE TABLE IF NOT EXISTS `".$wpdb->prefix . "woocommerce_bnpmercanetcw_storage` (
