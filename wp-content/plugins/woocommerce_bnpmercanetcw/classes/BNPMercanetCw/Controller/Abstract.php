 ...
 *
 */
class BNPMercanetCw_Controller_Abstract {

	protected function loadTransaction($parameters){
		if (!isset($parameters['cwtid'])) {
			throw new Exception(__('No transaction ID provided.', 'woocommerce_bnpmercanetcw'));
		}
		//Make sure payment methods are loaded
		BNPMercanetCw_Util::getPaymentMethods(true);
		$dbTransaction = BNPMercanetCw_Util::getTransactionById($parameters['cwtid'], false);

		if ($dbTransaction === null) {
			throw new Exception(__('Invalid transaction ID provided.', 'woocommerce_bnpmercanetcw'));
		}

		if (!isset($parameters['cwtt']) || $parameters['cwtt'] != BNPMercanetCw_Util::computeTransactionValidateHash($dbTransaction)) {
			throw new Exception(__('Missing Permissions.', 'woocommerce_bnpmercanetcw'));
		}
	...
	protected function loadOrder($parameters){
		if (!isset($parameters['cwoid'])) {
			throw new Exception(__('No order ID provided.', 'woocommerce_bnpmercanetcw'));
		}

		if (!isset($parameters['cwot']) || $parameters['cwot'] != BNPMercanetCw_Util::computeOrderValidationHash($parameters['cwoid'])) {
			throw new Exception(__('Missing Permissions.', 'woocommerce_bnpmercanetcw'));
		}
	...
	protected function getPaymentMethodModule($order){
		...
		return BNPMercanetCw_Util::getPaymentMehtodInstance($methodName);
	}
	 ...
	 */
	protected function validateTransaction(BNPMercanetCw_OrderContext $orderContext, Customweb_Payment_Authorization_IAdapter $authorizationAdapter, array $parameters){
		$errorMessage = null;
		...
		if ($errorMessage !== null) {
			...
			header(
					'Location: ' .
					Customweb_Util_Url::appendParameters(get_permalink(BNPMercanetCw_Util::getPermalinkIdModified($option)),
							array(
								'bnpmercanetcwove' => $errorMessage
							)));
			die();
		}
	}
	...
	protected function getAlias($parameters, $userId){
		...
		if (isset($parameters['cwalias'])) {
			$aliasTransaction = BNPMercanetCw_Util::getAliasTransactionObject($parameters['cwalias'], $userId);
		}
	...
	protected function getFailed($parameters){
		...
		if (isset($parameters['cwfail'])) {
			$failedTransaction = BNPMercanetCw_Util::getFailedTransactionObject($parameters['cwfail'], $parameters['cwfailtoken']);
		}
