 ...
 *
 */
class BNPMercanetCw_Controller_Iframe extends BNPMercanetCw_Controller_Abstract {
	...
	public function indexAction() {
		...
		if($dbTransaction->getTransactionObject()->isAuthorizationFailed()){
			$url = BNPMercanetCw_Util::getPluginUrl('failure', array('cwtid' => $dbTransaction->getTransactionId(), 'cwtt' => BNPMercanetCw_Util::computeTransactionValidateHash($dbTransaction)));
			header('Location: ' . $url);
			die();
		}
		
		ob_start();
		BNPMercanetCw_Util::includeTemplateFile('payment_iframe', $variables);
	...
	public function breakOutAction() {
		...
		$GLOBALS['woo_bnpmercanetcwTitle'] = __('Break Out' , 'woocommerce_bnpmercanetcw');
		...
		BNPMercanetCw_Util::includeTemplateFile('iframe_break_out', $variables);
		$content = ob_get_clean();
		echo $content;
		die();
	}
