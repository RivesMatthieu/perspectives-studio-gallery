 ...
 *
 */
class BNPMercanetCw_Controller_Success extends BNPMercanetCw_Controller_Abstract {
	...
	public function indexAction() {
		...
		$method = BNPMercanetCw_Util::getPaymentMehtodInstance($dbTransaction->getPaymentClass());
		...
		while (true) {
			...
			if ($transactionObject->isAuthorizationFailed()) {
				...
				$url = BNPMercanetCw_Util::getPluginUrl('failure', array('cwtid' => $parameters['cwtid'], 'cwtt' => $parameters['cwtt']));
			}
			...
			if (time() - $start > $maxExecutionTime) {
				ob_start();
				$GLOBALS['woo_bnpmercanetcwTitle'] = __('Time Out' , 'woocommerce_bnpmercanetcw');
				BNPMercanetCw_Util::includeTemplateFile('timeout', array('successUrl' => $successUrl));
