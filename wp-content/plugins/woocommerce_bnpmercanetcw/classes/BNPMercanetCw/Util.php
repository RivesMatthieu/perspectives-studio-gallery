 ...
 *
 */
BNPMercanetCw_Util::bootstrap();

require_once 'BNPMercanetCw/Database/LinkAccessor.php';
require_once 'BNPMercanetCw/EntityManager.php';
...
require_once 'BNPMercanetCw/ContextCleanUpBean.php';
...
require_once 'BNPMercanetCw/TransactionCleanUpBean.php';
require_once 'BNPMercanetCw/Util.php';
require_once 'Customweb/Core/Url.php';
require_once 'BNPMercanetCw/Entity/PaymentCustomerContext.php';
require_once 'BNPMercanetCw/LayoutRenderer.php';
...
class BNPMercanetCw_Util {
	 ...
	 * @return BNPMercanetCw_PaymentMethod
	 */
	...
	public static function getPluginUrl($controller, array $params = array(), $action = null){
		...
		if (isset($_REQUEST['woo-bnpmercanetcw-lang'])) {
			$params['woo-bnpmercanetcw-lang'] = $_REQUEST['woo-bnpmercanetcw-lang'];
		}		
		else if (defined('ICL_LANGUAGE_CODE')) {
			$params['woo-bnpmercanetcw-lang'] = ICL_LANGUAGE_CODE;
		}
		else {
			$tmpLang = apply_filters( 'wpml_current_language', '' );
			if(!empty($tmpLang)){
				$params['woo-bnpmercanetcw-lang'] = $tmpLang;
			}
		}
		...
		$urlString = get_permalink(get_option('woocommerce_bnpmercanetcw_page'));
		if(isset($params['woo-bnpmercanetcw-lang'])){
			$urlString = apply_filters('wpml_permalink', $urlString, $params['woo-bnpmercanetcw-lang']);
		}
		...
		return apply_filters('woocommerce_bnpmercanetcw_plugin_url', $complete, $url->getBaseUrl() . $url->getPath(), 
				$url->getQueryAsArray());
	}
	...
	public static function getPermalinkIdModified($id){
		...
		if (isset($_REQUEST['woo-bnpmercanetcw-lang'])) {
			$language = $_REQUEST['woo-bnpmercanetcw-lang'];
		}
		else if (defined('ICL_LANGUAGE_CODE')) {
	...
	private static function checkRequirements() {
		...
		if (!is_plugin_active('woocommerce/woocommerce.php')){
			$errors[] = sprintf(__("Woocommerce has to be active.", "woocommerce_bnpmercanetcw"));
		}
		else{
			...
			if (version_compare ($woocommerce_data['Version'] , '3.0.0', '<')){
				$errors[] = sprintf(__("Woocommerce %s+ is required. (You're running version %s)", "woocommerce_bnpmercanetcw"), '3.0.0', $woocommerce_data['Version']);
			}
		}		
		if(!empty($errors)){
			$title = __('Could not activate plugin WooCommerce BNPMercanetCw', 'woocommerce_bnpmercanetcw');
			$message = '<h1><strong>'.$title.'</strong></h1><br/>'.
					'<h3>'.__('Please check the following requirements before activating:', 'woocommerce_bnpmercanetcw').'</h3>'.
					'<ul><li>'.
	...
	public static function installPlugin(){
		...
		$manager = new Customweb_Database_Migration_Manager(self::getDriver(), dirname(__FILE__) . '/Migration/', 
				$wpdb->prefix . 'woocommerce_bnpmercanetcw_schema_version');
		$manager->migrate();
		
		//Create Page
		$optionValue = get_option('woocommerce_bnpmercanetcw_page');
		$pageContent = '[woocommerce_bnpmercanetcw]';
		$pageSlug = 'woo_bnpmercanetcw';
		$pageTitle = 'BNP Paribas Mercanet Checkout';
		...
		update_option('woocommerce_bnpmercanetcw_page', $pageId);
	}
	...
	public static function includeTemplateFile($templateName, $variables = array()){
		...
		$templateName = 'bnpmercanetcw_' . $templateName;
	...
	public static function createContainer(){
		if (self::$container === null) {
			$packages = array(
			0 => 'Customweb_BNPMercanet',
 			1 => 'Customweb_Payment_Authorization',
 		);
			$packages[] = 'BNPMercanetCw_';
			...
			// @formatter:off
			$storage = new Customweb_Storage_Backend_Database(self::getEntityManager(), self::getDriver(), 'BNPMercanetCw_Entity_Storage');
			$provider->addObject(BNPMercanetCw_ContextRequest::getInstance())
				->addObject(self::getEntityManager())
				->addObject(self::getDriver())
				->addObject(new BNPMercanetCw_LayoutRenderer())
				->addObject(new Customweb_Cache_Backend_Memory())
				->add('databaseTransactionClassName', 'BNPMercanetCw_Entity_Transaction')
				->addObject(self::getAssetResolver())
				->addObject($storage)
				->addObject(new BNPMercanetCw_ContextCleanUpBean(self::getEntityManager()))
				->addObject(new BNPMercanetCw_TransactionCleanUpBean(self::getEntityManager()));
			// @formatter:om
	...
	public static function getEntityManager(){
		if (self::$entityManager === null) {
			$cache = new Customweb_Cache_Backend_Memory();
			self::$entityManager = new BNPMercanetCw_EntityManager(self::getDriver(), $cache);
		}
	...
	public static function getAssetResolver(){
		$simple = array();
		$simple[] = new Customweb_Asset_Resolver_Simple(get_template_directory(). '/woocommerce_bnpmercanetcw/', null,
				array(
					'application/x-phtml'
				));
		...
		$simple[] = new Customweb_Asset_Resolver_Simple(get_template_directory(). '/woocommerce_bnpmercanetcw/', get_stylesheet_directory_uri() . '/woocommerce_bnpmercanetcw/');
	...
	public static function getDriver(){
		if (self::$driver === null) {
			global $wpdb;
			$link = BNPMercanetCw_Database_LinkAccessor::getBNPMercanetCwLink($wpdb);
	...
	public static function getPaymentCustomerContext($customerId){
		...
		if (!isset(self::$paymentCustomerContexts[$customerId])) {
			$entities = self::getEntityManager()->searchByFilterName('BNPMercanetCw_Entity_PaymentCustomerContext', 'loadByCustomerId', 
					array(
			...
			if (count($entities) > 0) {
				...
				$context = new BNPMercanetCw_Entity_PaymentCustomerContext();
	...
	public static function persistPaymentCustomerContext(Customweb_Payment_Authorization_IPaymentCustomerContext $context){
		if ($context instanceof BNPMercanetCw_Entity_PaymentCustomerContext) {
	...
	public static function thankYouPageHtml($orderId){
		...
		if (!empty($paymentInformation)) {
			echo '<div class="woocommerce_bnpmercanetcw-payment-information" id="woocommerce_bnpmercanetcw-payment-information">';
			echo "<h2>" . __('Payment Information', 'woocommerce_bnpmercanetcw') . "</h2>";
	...
	public static function orderEmailHtml($order, $sent_to_admin, $plain_text = false){		
		...
		$paymentInformation = trim($transactionObject->getPaymentInformation());		
		if(!empty($paymentInformation)) {
			echo '<div class="woocommerce_bnpmercanetcw-email-payment-information" id="woocommerce_bnpmercanetcw-email-payment-information">';
			echo "<h2>" . __('Payment Information', 'woocommerce_bnpmercanetcw') . "</h2>";
			echo $paymentInformation;
			echo '</div>';
		}
	 ...
	 * @return BNPMercanetCw_Entity_Transaction The matching transactions for the given transaction id
	 */
	public static function getTransactionById($id, $cache = true){
		return self::getEntityManager()->fetch('BNPMercanetCw_Entity_Transaction', $id, $cache);
	}
	 ...
	 * @param boolean $cache load from cache
	 * @return BNPMercanetCw_Entity_Transaction The matching transactions for the given transactionNumber
	 */
	public static function getTransactionByTransactionNumber($number, $cache = true){
		$transactions = self::getEntityManager()->searchByFilterName('BNPMercanetCw_Entity_Transaction', 'loadByExternalId', 
				array(
	 ...
	 * @param boolean $cache load from cache
	 * @return BNPMercanetCw_Entity_Transaction[] The matching transactions for the given order id
	 */
	public static function getTransactionsByOrderId($orderId, $cache = true){
		self::getPaymentMethods(true);
		return self::getEntityManager()->searchByFilterName('BNPMercanetCw_Entity_Transaction', 'loadByOrderId', 
				array(
					'>orderId' => $orderId 
				), $cache);
	}
	...
	public static function getTransactionsByPostId($postId, $cache = true){
		self::getPaymentMethods(true);
		return self::getEntityManager()->searchByFilterName('BNPMercanetCw_Entity_Transaction', 'loadByPostId',
				array(
					'>postId' => $postId
				), $cache);
	}
	...
	public static function getAliasTransactions($userId, $paymentMethod){
		...
		$aliases = array();
		$entities = self::getEntityManager()->search('BNPMercanetCw_Entity_Transaction', 
	...
	public static function computeTransactionValidateHash(BNPMercanetCw_Entity_Transaction $transaction) {
	...
	public static function tryToParseDate($date_string){
		$date_of_birth = false;
		$custom_date_of_birth_format = apply_filters('woocommerce_bnpmercanetcw_custom_date_of_birth_format', '');
